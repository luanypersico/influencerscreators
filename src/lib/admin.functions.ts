...
export const adminSetPasswordFn = createServerFn({ method: "POST" })
  ...
  .inputValidator((data: { userId: string; password: string }) => {
    if (!data?.userId) throw new Error("Usuário inválido.");
    if (!data?.password || data.password.length < 8) throw new Error("Senha muito curta (mínimo 8).");
    return { userId: data.userId, password: data.password };
  })
  .handler(async ({ context, data }) => {
    await assertSuperAdmin(context.userId);
    await setUserPassword({ actorId: context.userId, userId: data.userId, password: data.password });
    return { ok: true };
  });
...
export const adminSetAccessFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { userId: string; productId: string; enabled: boolean; expiresAt?: string | null }) => {
...
export const adminPreviewAudienceFn = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { audience: "all" | "buyers" | "leads" | "product" | "manual"; productId?: string | null; manual?: string[] }) => ({
...
export const adminSendEmailFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (recipients.length === 0) throw new Error("Nenhum destinatário encontrado para este público.");
    return sendEmails({
      actorId: context.userId,
      recipients,
      subject: data.subject,
